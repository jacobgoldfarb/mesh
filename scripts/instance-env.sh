...
BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"ai.superhuman.mesh.app.dev\",\"productName\":\"Superhuman Mesh Dev\"}"
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    if [[ -n "$GIT_COMMON_DIR" && "$GIT_DIR" != "$GIT_COMMON_DIR" ]]; then
        ...
        if [[ "${BUZZ_SHARE_IDENTITY:-0}" == "1" ]]; then
            KEYRING_SERVICE="superhuman-mesh-desktop-dev"
            KEYRING_BLOB=""
            case "$(uname -s)" in
                Darwin)
            ...
            CANONICAL_KEY="$HOME/Library/Application Support/ai.superhuman.mesh.app.dev/identity.key"
            ...
            if [[ -n "$SHARED_IDENTITY" ]]; then
                ...
                echo "⚠ BUZZ_SHARE_IDENTITY=1 but no identity found in keyring service $KEYRING_SERVICE, at $CANONICAL_KEY, or at $LEGACY_CANONICAL_KEY — run Superhuman Mesh from repo root first" >&2
            fi
        fi
        ...
        if swift "$GENERATE_DEV_ICON" "$BASE_ICON" "$DEV_ICON" "$BUZZ_WORKTREE_LABEL"; then
            ...
            BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"ai.superhuman.mesh.app.dev.${BUZZ_INSTANCE_SLUG}\",\"productName\":\"Mesh Dev (${BUZZ_WORKTREE_LABEL})\",\"bundle\":{\"icon\":[\"$DEV_ICON\"]}}"
        fi
    fi
fi
