...
if [[ "$instance_id" != "ai.superhuman.mesh.app.dev" && "$instance_id" != ai.superhuman.mesh.app.dev.* ]]; then
    echo "reset-desktop-standalone-state: refusing non-dev bundle identifier: $instance_id" >&2
    exit 1
fi
if [[ "$keyring_service" != "superhuman-mesh-desktop-dev" && "$keyring_service" != superhuman-mesh-desktop-dev.* ]]; then
