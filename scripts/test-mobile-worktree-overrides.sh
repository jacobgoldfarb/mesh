...
grep -q '^BUNDLE_IDENTIFIER = ai\.superhuman\.mesh\.mobile\.feature-work-1$' "$ios" \
...
grep -q '^APP_DISPLAY_NAME = Mesh (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = Mesh \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = ai\.superhuman\.mesh\.mobile$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = ai.superhuman.mesh.mobile"
grep -q '^APP_DISPLAY_NAME = Mesh$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = Mesh"
...
grep -q 'resValue("string", "app_name", "Mesh")' "$gradle" \
  && pass "Gradle default app_name stays Mesh" \
...
if [[ "$3 $4 $5" == "shell pm list" ]]; then
  printf 'package:ai.superhuman.mesh.mobile\n'
  printf 'package:ai.superhuman.mesh.mobile.feature_work_1\n'
  printf 'package:ai.superhuman.mesh.mobile.w_2fast\n'
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*ai\.superhuman\.mesh\.mobile$'; then
