...
/// Parse the query string of a `mesh://message?…` URL into the JSON
...
/// Parse the query string of a `mesh://join?…` URL into the JSON payload
...
/// Hosts of the `mesh://` git-entity links built by
...
/// Validate a `mesh://repo|project|pr|issue?…` link and return it verbatim
...
///
/// Workspace tabs addressable by `mesh://repo|project` links — mirrors
...
/// Handle an incoming `mesh://` deep link URL.
///
/// Currently supports:
/// - `mesh://connect?relay=<ws(s)://...>` — emits `deep-link-connect` to the frontend
/// - `mesh://repo|project|pr|issue?…` — emits `deep-link-entity` to the frontend
pub(crate) fn handle_deep_link_url(app: &tauri::AppHandle, url_str: &str) {
    ...
    match url.host_str() {
        ...
        Some("join") => {
            // `mesh://join?relay=<ws(s)://...>&code=<invite code>` — fired by
        ...
        Some("repo" | "project" | "pr" | "issue") => {
            // `mesh://repo|project?owner=<pubkey>&d=<dtag>` and
            // `mesh://pr|issue?id=<eventId>&owner=<pubkey>&d=<dtag>` — the
