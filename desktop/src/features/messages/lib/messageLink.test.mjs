...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `mesh://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `mesh://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `mesh://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects mesh:// with wrong host", () => {
  const r = parseMessageLink(`mesh://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`mesh://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`mesh://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy mesh://message links", () => {
  const r = parseMessageLink(`mesh://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("isMessageLink matches mesh://message and legacy mesh://message", () => {
  assert.equal(
    isMessageLink(`mesh://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`mesh://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("mesh://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("mesh://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `mesh://message?channel=${CHANNEL}&id=${MESSAGE}`;
