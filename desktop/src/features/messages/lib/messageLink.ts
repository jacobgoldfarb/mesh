/**
 * `mesh://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `mesh://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "mesh:";
...
/**
 * Build a `mesh://message` URL for a given channel + message.
 *
...
/**
 * Parse a `mesh://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("mesh://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("mesh://message?") || href === "mesh://message";
}
 ...
 * CommonMark autolinks (`<mesh://message?...>`) and explicitly labeled links
