...
test("Mesh shared compute maps persisted auto to Default and hides custom input", () => {
...
test("Mesh shared compute fallback is Default auto while normal providers remain unchanged", () => {
...
test("Mesh shared compute keeps Default auto when discovery is empty", () => {
  const state = relayMeshModelPickerState({
    discoveredOptions: [],
    fallbackOptions: fallback,
    isCustomEditing: false,
...
test("Mesh shared compute dropdown contains Default plus live models and no custom escape hatch", () => {
