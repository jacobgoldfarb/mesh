...
/**
 * Payload emitted by the Rust deep-link handler for `mesh://message?…`.
...
/**
 * Payload emitted by the Rust deep-link handler for `mesh://join?…` —
 ...
 *
 * When a `mesh://connect?relay=<url>` link is opened, the handler
 ...
 *
 * When a `mesh://join?relay=<url>&code=<invite>` link is opened (relay
 ...
 *
 * `mesh://message?…` is handled separately by `listenForNavigationDeepLinks`,
...
/**
 * Register a listener for `deep-link-entity` events — the `mesh://` share
