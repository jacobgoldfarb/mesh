...
/**
 * The shipped mock bridge throws on every `terminal_*` command, so Mesh Term is
...
async function reveal(page: Page) {
  ...
  // Mesh Term needs a channel: TerminalBootstrap's context is null on Home, so
...
test("scrollback: wheel over Mesh Term reaches terminal_scroll", async ({
  page,
}) => {
  await reveal(page);
